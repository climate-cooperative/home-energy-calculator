...
const handleCalculation = async (formData) => {
  try {
    ...
    const apiData = await getAPIData(
      ...
      formData.laundry,
      formData.homeBuilt,
      formData.primaryHeat,
      formData.coolingSystem,
...
function CO2Emission(formdata, apiData) {
  ...
  const window_tinting_coeff = treatments.includes('Tinting / Smart Glass') ? 0.5 : 1;
  const south_facing_window = sun === 'Yes' ? 0.4 : sun === 'No' ? 0.2 : 0.25;
  const window_coverage = windows === 'Low' ? 0.12 : windows === 'Medium' ? 0.16 : 0.2;

  const insulation_r = insulation.includes('Walls')
    ? home_decades.recent['wall_insulation_r']
    : home_decades.actual['wall_insulation_r'];
  const attic_r = insulation.includes('Attic')
    ? home_decades.recent['attic_insulation_r']
    : home_decades.actual['attic_insulation_r'];

  const led_lighting = energy.includes('All')
    ? 1
    : energy.includes('Most')
      ? 0.8
      : energy.includes('Some')
        ? 0.6
        : energy.includes('Not Sure')
          ? 0.35
          : 0;
  ...
  const solar_heat_gain_summer = solarHeatGain(
  ...
  const secondary_conversion =
    secondarySource !== '' ? getConversionFactor(secondarySource, grid_carbon_intensity) : 0;
  const carbon_intensity_primary_heating = carbonIntensityHeating(
    hvac_heating_efficiency,
    primary_conversion
  ); // fix
  const carbon_intensity_secondary_heating = carbonIntensityHeating(
    hvac_heating_efficiency,
    secondary_conversion
  ); // fix
  const solar_heat_gain_winter = solarHeatGain(
    solar_insolation_winter,
    house_window_exposed,
    coeff_solar_heat_gain
  );
  const rvalue_walls = rvalueWalls(
    r_probability_insulation,
    r_wall_construction,
    insulation_r,
    r_wall_siding
  ...
  const carbon_intensity_heating_total = carbonIntensityHeatingTotal(
    coeff_primary_heating,
    carbon_intensity_primary_heating,
    carbon_intensity_secondary_heating
  );
  const carbon_intensity_water_heating = carbonIntensityWaterHeating(
    grid_carbon_intensity,
    hvac_water_heating_efficiency
  );
  ...
  const btu_heating_through_windows = btuHeatingThroughSurface(
    sqft_windows,
    region_hdd,
    rvalue_windows
  );
  ...
  const btu_cooling_through_windows = btuHeatingThroughSurface(
    sqft_windows,
    region_cdd,
    rvalue_windows
  );
  ...
  const btu_heating = btuHeatingOrCooling(
    surface_heat_gain,
    infiltration_heat_gain,
    solar_heat_gain_summer
  );
  const btu_cooling = btuHeatingOrCooling(
    surface_heat_loss,
    infiltration_heat_loss,
    solar_heat_gain_winter
  );
  const btu_water_heating = btuWaterHeating(
    region_water_temp,
    water_heating_modifier,
    rooms.Bedrooms + rooms.Bathrooms
  ); //regionGroundwaterTemp, waterHeatingModifier, bedrooms, bathrooms
  const co2_electric_appliances = co2ElectricAppliances(
    rooms.Kitchens,
    rooms.bedroom,
    btu_electric_appliances,
    grid_carbon_intensity
  ...
  const co2_heating = co2Heating(
    heatedFloors.Rooms,
    btu_heating,
    carbon_intensity_heating_total,
    grid_carbon_intensity
  );
  const co2_total = Math.floor(
    co2Total(
      co2_heating,
      co2_cooling,
      co2_water_heating,
...
function getGrades(
  ...
  const { primarySource, fuelSource, kitchen, laundry, windows, efficiency, rooms } = formData;
  const {
    hvac_heating_efficiency,
    hvac_cooling_efficiency,
    hvac_water_heating_efficiency,
  ...
  const water_heating_all = getGrade(
    2 +
      water_heating_efficiency +
      water_heating_fuel +
      water_heating_local_grid +
      water_heating_local_climate
  );
  // appliances
  const appliances_efficiency = gradeAppliancesUnitEfficiency(
    kitchen['Induction Cooktop'],
    laundry['Heat Pump Dryer']
  );
  const appliances_fuel = gradeAppliancesFuel(kitchen, laundry);
  const appliances_local_grid = gradeAppliancesLocalGrid(
    appliances_fuel > 0,
    grid_carbon_intensity
  );
  const appliances_other = gradeAppliancesOther(rooms.Kitchens);
  const appliances_all = getGrade(
    2 + appliances_efficiency + appliances_fuel + appliances_local_grid + appliances_other
  );
  ...
  const window_coverage = gradeWindowCoverage(
    windows === 'Low' ? 0.12 : windows === 'Medium' ? 0.16 : 0.2
  );
  ...
  return [
      ...
      content: [
        {
          ...
          icon: hvac_efficiency === 3 ? checkmark : hvac_efficiency === 1 ? alert : close
        },
      ...
      content: [
        {
          title: 'Unit Efficiency',
          icon:
            water_heating_efficiency === 2
              ? checkmark
              : water_heating_efficiency === 1
                ? alert
                : close
        },
        {
          title: 'Ground Water Temp.',
          icon: water_heating_local_climate === 1 ? checkmark : close
        },
        {
          title: 'Fuel Source',
          icon: water_heating_fuel === 2 ? checkmark : water_heating_fuel === 1 ? alert : close
        },
    ...
    },
    {
      ...
      content: [
        {
          title: 'High Efficiency Appliance',
          icon: appliances_efficiency === 0 ? close : checkmark
        },
        {
          title: 'Electric Appliances',
          icon: appliances_fuel === 2 ? checkmark : appliances_fuel === 1 ? alert : close
        },
    ...
    },
    {
      ...
      content: [
        {
          title: 'Sealing',
          icon: sealing_ach === 4 ? checkmark : sealing_ach === 2 ? alert : close
        },
        {
          title: 'Insulation',
          icon: insulation === 4 ? checkmark : insulation === 2 ? alert : close
        }
      ],
      rows: [
        { name: 'Add Attic Insulation', userValue: 'Save $4', recommendedValue: 'Down 100 lbs' },
        {
          name: 'Add Crawl Space Insulation',
          userValue: 'Save $4',
          recommendedValue: 'Down 4 lbs'
        },
    ...
    },
    {
      ...
      content: [
        {
          title: 'Efficient Bulbs',
          icon: led_lighting === 8 ? checkmark : led_lighting <= 5 ? close : alert
        }
      ],
      rows: [
